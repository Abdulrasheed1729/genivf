...
float Point::operator[](size_t index) const { return values[index]; }

IndexIVF::IndexIVF(size_t num_cells, size_t dim, unsigned seed)
    : d_num_cells(num_cells), d_dim(dim), d_seed(seed) {
  if (num_cells <= 0) {
...
IndexIVF::IndexIVF(size_t num_cells, size_t dim)
    : d_num_cells(num_cells), d_dim(dim) {
  if (num_cells <= 0) {
    throw std::invalid_argument("IndexIVF: num_cells must be > 0");
  }
  if (dim == 0) {
...
double IndexIVF::distance_l2_sq(const Point &a, const Point &b) {
...
double IndexIVF::distance_l2(const Point &a, const Point &b) {
  return std::sqrt(distance_l2_sq(a, b));
}

size_t IndexIVF::find_nearest_centroid(const Point &point) const {
  size_t nearest = 0;
  double min_dist_sq =
      distance_l2_sq(point, this->d_clusters[0].centroid);
...
void IndexIVF::train(std::span<const Point> points, size_t max_iter,
                     double epsilon) {
  ...
  for (const auto &p : points) {
    if (p.values.size() != d_dim) {
      throw std::invalid_argument(
          "IndexIVF::train: point dimension does not match index dimension");
    }
  }
  ...
  // HACK: Pre-allocate new_centroids outside the loop — avoids reallocating the
  // outer vector on every iteration.
  ...
  for (size_t iter = 0; iter < max_iter; ++iter) {
    ...
    for (size_t i = 0; i < this->d_num_cells; ++i) {
      if (assignments[i].empty()) {
        // A cluster lost all its members — keep the old centroid in place.
      ...
      // Compare against epsilon^2 — same threshold, no transcendental call.
...
void IndexIVF::add(std::span<const Point> points) {
  if (!is_trained()) {
  ...
  for (const auto &point : points) {
    ...
    // Detect duplicate IDs before inserting the emplace() silently no-ops on a
    // collision, which would leave the inverted list pointing at the old
    // vector.
...
std::vector<SearchResult> IndexIVF::search(const Point &query, size_t k,
                                           size_t nprobe) const {
  if (!this->is_trained()) {
    throw std::logic_error("IndexIVF::search: call train() before search()");
  }
