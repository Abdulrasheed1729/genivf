...
struct Point
{
    ...
    Point(size_t id, std::initializer_list<float> args)
      : values(args)
      , id(id)
    {
    }
...
struct IndexIVF
{
    ...
    // improves recall but increases cost. Must satisfy 1 <= nprobe <=
    // num_cells.
    [[nodiscard]] std::vector<SearchResult> search(const Point& query,
                                                   size_t k,
