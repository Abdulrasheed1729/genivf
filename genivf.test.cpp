...
TEST_CASE("IndexIVF instance: should throw with `dim = 0` or `num_cells <= 0`")
{
    size_t num_cells = 0;
    size_t dim = 0;
    CHECK_THROWS_AS(genivf::IndexIVF ivf(num_cells, dim),
...
TEST_CASE("test the distance l2 and l2 sq.")
{
    ...
    CHECK_EQ(genivf::IndexIVF::distance_l2(p1, p2), 2.0 * std::sqrt(2.0));
...
TEST_CASE("test out the distance l2")
{
    std::vector<genivf::Point> points = {
        ...
        { 3, { 7.0, 8.0, 9.0 } },    { 4, { 10.0, 11.0, 12.0 } },
        { 5, { 13.0, 14.0, 15.0 } }, { 6, { 16.0, 17.0, 18.0 } },
    };
}
...
// and there should be some kind of exception throw while calling add before
// train The train and add functions seems kind of hard to test, i don't know
// probably due to floating points, I don't know.
// NOTE: so what I am thinking of
// doing right now is to just train wih Dengue virus sequence and use python
// with a tsv file i generated and use seaborn or matplotlib for visualisation.
